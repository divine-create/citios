...
test('HotelOS Concurrency & Security Regression', async (t) => {
  ...
  await t.test('createReservation uses db.transaction and FOR UPDATE to prevent concurrent double bookings', () => {
    ...
    assert.match(code, /SELECT.*HotelRoom.*FOR UPDATE/i, 'Must lock HotelRoom row using FOR UPDATE');
    assert.match(code, /SELECT id FROM "Reservation"/i, 'Must query overlaps within transaction');
  });
  ...
  await t.test('settleFolio uses db.transaction and Wallet FOR UPDATE to prevent financial race conditions', () => {
    ...
    assert.match(code, /SELECT.*Wallet.*FOR UPDATE/i, 'settleFolio must lock Wallets');
  ...
  await t.test('IDOR and tenant isolation is enforced', () => {
    assert.match(code, /await requireMembership.*getFolio/s, 'getFolio must authorize');
    assert.match(code, /await requireMembership.*updateRoom/s, 'updateRoom must authorize');
    assert.match(code, /await requireMembership.*updateReservationStatus/s, 'updateReservationStatus must authorize');
  });
});
