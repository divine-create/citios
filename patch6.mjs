...
const startStr = 'export async function createOrder(input: {';
const endStr = 'async function enrichOrders(';
...
if (startIdx !== -1) {
  ...
  try {
    ...
    if (input.locationId) {
      const loc = await db.orm.public.Location.where({ id: input.locationId }).all().first();
      if (!loc || loc.organizationId !== input.organizationId) {
    ...
    if (input.shiftId) {
      ...
      if (input.locationId && shift.locationId && shift.locationId !== input.locationId) {
...
`;
    ...
    console.log('Successfully replaced createOrder');
  } else {
